...
# -*- coding: utf-8 -*-
"""
Created on Sat Jun 18 22:20:29 2022
...
MyNetwork.add(tf.keras.layers.Conv2D(
        filters=32,             # 32 noyaux de convolutions (32 feature maps)
        kernel_size=(3,3),     # noyau de convolution 5x5
        strides=(1,1),         # décalages horizontal=1 / vertical=1
        activation='relu',     # fct d'activation=relu
...
MyNetwork.add(tf.keras.layers.AveragePooling2D(
        pool_size=(3,3),       # noyau de pooling 2x2
        strides=(2,2),         # décalages horizontal=2 / vertical=2
        padding='valid'))      # pas d'ajout de bord à l'image    
...
# Création d'un couche de 200 neurones avec fonction d'activation relu
MyNetwork.add(tf.keras.layers.Dense(200, activation='relu'))

# FC6: connexion totale avec couche de 84 neurones avec fct d'activation relu
#MyNetwork.add(tf.keras.layers.Dense(84, activation='relu'))

#dropout fuction 
MyNetwork.add(tf.keras.layers.Dropout(0.25))

# Sortie: 10 neurones avec fct d'activation Softmax
MyNetwork.add(tf.keras.layers.Dense(10, activation='softmax'))
...
perf=MyNetwork.evaluate(x=x_test, y=y_test)
